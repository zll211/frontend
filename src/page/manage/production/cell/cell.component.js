...
import cellData, {tabList} from './cell.constant';
...
export default {
  ...
  methods: {
    statusList() {
      productionService.statusList('cell')
        .then(({body}) => {
    ...
    confirmOperate({status, rows, childStatus, prodParams}) {
      ...
      productionService.updateStatus('cell', params)
        .then((res) => {
    ...
    prodList({page = 1, page_size = this.pagination.per_page, status = this.tabList[0].label, ...rest} = {
      ...
      this.label = status;
      this.prods = [];
      productionService.prodList('cell', Object.assign({
        status: status === '全部' ?
    ...
    updateStatus() {
      ...
      let params = {
        section_at: formatDateTime(this.productionForm?.startTime),
        patch_at: formatDateTime(this.productionForm?.endTime),
    ...
    printLabel({rows, size}) {
      ...
      productionService.printLabel('cell', params)
        .then((res) => {
    ...
    getTableCount() {
      productionService.getTableCount('cell').then((res) => {
