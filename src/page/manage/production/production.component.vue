    ...
    <pis-tab v-model="activeTabName"
      ...
      <el-row>
        ...
        <el-button type="danger" size="mini"
                   @click="backOperate()"
                   v-show="activeTabName !== 'recut'&&activeTabName !== 'finish'&&activeTabName !== 'total'&&activeTabName!==tabList[0].name">
          回退
        </el-button>
        <el-button @click="printLabel()" type="primary" size="mini">标签打印</el-button>
        ...
        <template slot-scope="scope">
          <div class="flex align-items-center justify-content-center wrap">
            ...
            <el-button style="margin: 5px" type="danger" size="mini"
                       ...
                       v-if="activeTabName !== 'recut'&&activeTabName !== 'finish'&&activeTabName !== 'total'&&activeTabName!==tabList[0].name">
              回退
            </el-button>
            <el-button style="margin: 5px" size="mini"
                       @click.stop="recordEntrapment(scope.row.id)"
                       v-if="activeTabName === 'entrapment'">
              记录
            </el-button>
            <el-button style="margin: 5px" type="primary" size="mini"
            ...
            <el-button style="margin: 5px" @click.stop="printSection(scope.row)" type="primary" size="mini" v-show="activeTabName === 'finish'">切片工作单打印</el-button>
          </div>
        </template>
      </el-table-column>
    </el-table>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      operateWidth() {
        if (this.activeTabName === 'dehydration' || this.activeTabName === 'recut' || this.activeTabName === 'production') {
          return 100;
        } else if(this.activeTabName === 'entrapment'){
          return 250;
        } else {
          return 180;
        }
      ...
      statusOptions() {
        if (this.activeTabName === 'recut') {
          let index = this.tabList.findIndex((tab) => tab.name === 'section');
          this.status = '';
          return [...this.tabList.slice(index + 1, this.tabList.length - 1)];
        } else {
          let index = this.tabList.findIndex((tab) => tab.name === this.activeTabName);
          this.status = '';
          if (this.tabList.find((tab) => tab.name === 'recut')) {
            return this.tabList.slice(0, index).concat(this.tabList.slice(index + 1, this.tabList.length - 1));
          } else {
      ...
      prodList() {
        ...
        this.prods.forEach((prod) => {
          prods = [...prods, ...prod.production.map((item, index) => {
            if (index === 0) item.count = prod.production.length;
