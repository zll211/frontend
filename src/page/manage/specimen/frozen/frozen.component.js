...
import {fSpecimenService} from "./frozen.service";
  ...
  methods: {
    ...
    deleteSpecimen(id) {
      messageBoxService.delete('是否删除该条记录?','删除后不可恢复')
        .then(() => {
          fSpecimenService.delRegister(id).then((res) =>{
            this.$message({
              type: 'success',
              message: '删除成功!'
            });
    ...
    getTable({page = 1, page_size = this.tablePageSize,...rest} = {  // 获取列表
      ...
      fSpecimenService.getRegisterTable(Object.assign({
        page: page,
        page_size: page_size,
        case_type: '冰冻',
    ...
    getTableCount () {
      fSpecimenService.getTableCount().then((res) =>{
