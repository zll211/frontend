...
export default {
  ...
  data() {
    return {
      ...
      tablePageSize: 10,
      logs: [],
      activeName: 'small_specimen_timeout',
      countList: {},
    };
  },
  created() {
    if(Object.keys(this.$route.query)[0]) {
      this.activeName = Object.keys(this.$route.query)[0];
    }
  },
  mounted: function () {
    this.getTable();
  ...
  methods: {
    ...
    getTable({page = 1, page_size = this.tablePageSize,...rest} = {  // 获取列表
      ...
      timeoutService.getTable(Object.assign({
        page: page,
        page_size: page_size,
        search: this.specimenSearchInput,
        timeout_type: this.activeName,
        ...
        .then((res) => {
          ...
          this.tableData = res.body.data.map((item)=>{
            return {
              ...
              has_use_time: item.has_use_time,
    ...
    getTableCount () {
      timeoutService.timeoutTableCount().then((res) =>{
